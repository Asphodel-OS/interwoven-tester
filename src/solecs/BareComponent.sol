...
abstract contract BareComponent is IComponent, OwnableWritable {
	 ...
	 */
	function registerWorld(address _world) public virtual onlyOwner {
		world = _world;
	 ...
	 */
	function extractRawBatch(uint256[] memory entities) public virtual override onlyWriter returns (bytes[] memory) {
	 ...
	 */
	function getRawBatch(uint256[] memory entities) public view virtual override returns (bytes[] memory) {
	...
	function _setBatch(uint256[] memory entities, bytes[] memory values) internal virtual {
		...
		for (uint256 i = 0; i < entities.length; i++) currWorld.registerComponentValueSet(entities[i], values[i]);
	}
