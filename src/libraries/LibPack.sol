...
library LibPack {
	...
	function unpackArr(uint256 packed, uint256 numElements, uint256 SIZE) internal pure returns (uint32[] memory) {
	...
	function pUpdateAt(
		...
		return
			(packed & ~(((1 << SIZE) - 1) << (SIZE * (maxPos - position)))) | (newElement << (SIZE * (maxPos - position)));
	}

	/// @notice gets a bitpacked value at a specific position
	function pGetAt(uint256 packed, uint256 position, uint256 maxPos, uint256 SIZE) internal pure returns (uint256) {
		return packed & (((1 << SIZE) - 1) << (SIZE * (maxPos - position)));
	}

	function pUpdateAtU32(uint32 newElement, uint256 position, uint256 packed) internal pure returns (uint256) {
